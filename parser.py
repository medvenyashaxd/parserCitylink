...
def search_data():
    ...
    while number_page != 1 + 1:  # Кол-во страниц для парсинга
        ...
        products = soup_catalog.findAll('a',
                                        class_='ProductCardHorizontal__title Link js--Link Link_type_default')  # Находим товары в каталоге
        ...
        for product in products:  # В цикле разбираем каждый товар
            ...
            try:  # Пробуем получить данные
                product_id = soup_card.find('div', class_='ProductPageUsersActivity__product-id').text.split(':')[1].strip()  # Получаем код товара
                color = soup_card.find('h1', 'Heading Heading_level_1 ProductPageTitleSection__text').text.split(',')[2].strip()
                ...
                photography = []
                photos = soup_card.findAll('img', class_='ProductPageStickyGallery-gallery__image-upper PreviewList__image Image')  # Получаем фотографии
                for p in photos:
                ...
                print(product_id, brand, model, color, photography, specification[2:])
...
def start_pars():
    ...
    for data in search_data():  # Производим запись данных в файл
        ...
        page.write(row, column+5, str(data[5][2:]).replace('[', '').replace(']', '').replace("'", ''))
